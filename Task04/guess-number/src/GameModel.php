...
class GameModel
{
    ...
    {
        $db = new SQLite3($this->db_path);
        $sql = "SELECT playerName, COUNT(CASE WHEN gameResult = 'won' THEN 1 END) AS wins, COUNT(CASE WHEN gameResult = 'lose' THEN 1 END) AS losses FROM GamesInfo GROUP BY playerName ORDER BY wins DESC";
        $result = $db->query($sql);
        $players = [];
    ...
    public function saveGameIntoDatabase($info)
    {
        $dbPath = 'gamedb.db';
